...
import { Bell, BellOff, Check, CheckCheck, RefreshCw, AlertTriangle, Info, TrendingDown } from 'lucide-react';
...
function typeIcon(type) {
  ...
  if (t.includes('high') || t.includes('risk') || t.includes('churn')) return TrendingDown;
...
function typeColor(type) {
  ...
  const t = type.toLowerCase();
  if (t.includes('high') || t.includes('churn')) return { bg: 'bg-red-50', text: 'text-red-600', ring: 'ring-red-100' };
...
export default function NotifikasiPage() {
  ...
  const fetchNotif = useCallback(async () => {
    ...
    try {
      ...
      // admin hanya lihat notif aktivitas dari staff (bukan notif penugasan ke staff)
      const adminList = list.filter(n => n.type !== 'assign');
      setNotifications(adminList.sort((a, b) => {
        const readA = a.is_read || a.read;
        const readB = b.is_read || b.read;
      ...
      {loading ? (
        ...
        <motion.div variants={stagger} className="space-y-3">
          <AnimatePresence mode="popLayout">
            {filtered.map((n) => (
              <NotifCard key={n.id} notif={n} onRead={handleRead} t={t} lang={lang} />
            ))}
          </AnimatePresence>
        </motion.div>
      )}
