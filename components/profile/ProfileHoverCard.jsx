...
function menuRowClass() {
  return 'flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[13px] font-medium text-slate-700 transition-colors hover:bg-slate-50';
}

/**
 * Menu akun ringkas: hover atau klik (tanpa pindah halaman sampai pilih aksi).
 * @param {{ placement?: 'top'|'bottom'|'right'|'left', interaction?: 'hover'|'click', children: React.ReactNode }} props
 */
export default function ProfileHoverCard({
  ...
  return (
    ...
    >
      ...
      {open ? (
        <div
          className={`absolute z-[80] w-[min(calc(100vw-24px),280px)] min-w-[248px] overflow-hidden rounded-2xl border border-slate-200/90 bg-white py-2 shadow-lg shadow-slate-900/12 ring-1 ring-slate-900/5 animate-in fade-in zoom-in-95 duration-150 ${placementClass[placement]}`}
          role="menu"
        ...
        >
          ...
          <nav className="relative px-2 pt-1">
            <Link href={settingsHref} className={menuRowClass()} role="menuitem" onClick={() => setOpen(false)}>
              <Settings className="h-4 w-4 shrink-0 text-slate-400" />
              Pengaturan
            </Link>
            <Link href={profileEditHref} className={menuRowClass()} role="menuitem" onClick={() => setOpen(false)}>
          ...
          <div className="relative mt-1 border-t border-slate-100 px-2 pb-1 pt-1">
            <button
              type="button"
              role="menuitem"
              className={`${menuRowClass()} w-full text-red-600 hover:bg-red-50 hover:text-red-700`}
              onClick={() => {
                setOpen(false);
                logout();
              }}
